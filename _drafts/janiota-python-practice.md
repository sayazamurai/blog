...
そこで、最年長のメンバーのニックネームを`oldest_member_nickname`という変数に保存し、`oldest_age`が更新された時に、同時に更新することにします。初期値として、空の文字列を定義しておきます。
...
嵐の最年長メンバーがリーダー大ちゃんであるというジャニヲタ的常識を、プログラミングを使って導き出すことができました💙
